...
AudioFramePacketizer::~AudioFramePacketizer()
{
    close();
    if (m_audioSend) {
        m_rtcAdaptor->destoryAudioSender(m_audioSend);
        m_audioSend = nullptr;
    }
}
...
bool AudioFramePacketizer::init()
{
    if (!m_audioSend) {
        ...
        rtc_adaptor::RtcAdaptor::Config sendConfig;
        sendConfig.rtp_listener = this;
        sendConfig.stats_listener = this;
        m_audioSend = m_rtcAdaptor->createAudioSender(sendConfig);
        return true;
    }
    return false;
}

void onAdaptorStats(const AdaptorStats& stats) {}

void onAdaptorData(char* data, int len)
{
    boost::shared_lock<boost::shared_mutex> lock(m_transport_mutex);
    if (audio_sink_) {
