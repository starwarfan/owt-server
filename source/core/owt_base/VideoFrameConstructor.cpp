...
VideoFrameConstructor::VideoFrameConstructor(VideoInfoListener* vil, uint32_t transportccExtId)
    ...
    , m_rtcAdaptor(RtcAdaptorFactory::CreateRtcAdaptor())
{
    ...
    , m_videoInfoListener(vil)
{
    m_config.transport_cc = transportccExtId;
    assert(base);
    m_rtcAdaptor = base->m_rtcAdaptor;
}
...
VideoFrameConstructor::~VideoFrameConstructor()
{
    ...
    unbindTransport();
    if (m_videoReceive) {
        m_rtcAdaptor->destoryVideoReceiver(m_videoReceive);
...
void VideoFrameConstructor::maybeCreateReceiveVideo(uint32_t ssrc) {
    if (!m_videoReceive) {
        ...
        rtc_adaptor::RtcAdaptor::Config recvConfig;
        recvConfig.ssrc = ssrc;
        recvConfig.transport_cc = transportccExtId;
        recvConfig.rtp_listener = this;
        recvConfig.stats_listener = this;
        recvConfig.frame_listener = this;

        m_videoReceive = m_rtcAdaptor->createVideoReceiver(recvConfig);
    }
}
...
void VideoFrameConstructor::onAdaptorFrame(const Frame& frame) {
    deliverFrame(frame);
}

void VideoFrameConstructor::onAdaptorStats(const AdaptorStats& stats) {
...
void VideoFrameConstructor::onAdaptorData(char* data, int len) {
