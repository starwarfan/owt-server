...
VideoSendAdaptorImpl::VideoSendAdaptorImpl(
    CallOwner* owner,
    const RtcAdaptor::Config& config)
    : m_enableDump(false)
    , m_config(config)
    ...
    m_taskRunner.reset(new owt_base::WebRTCTaskRunner("VideoSendAdaptorImpl"));
    m_taskRunner->Start();
    init();
}
...
bool VideoSendAdaptorImpl::init()
{
...
void VideoSendAdaptorImpl::reset() {
    m_keyFrameArrived = false;
    m_timeStampOffset = 0;
}

void VideoSendAdaptorImpl::onFrame(const Frame& frame) {
...
int VideoSendAdaptorImpl::onRtcpData(char* data, int len) {
    boost::shared_lock<boost::shared_mutex> lock(m_rtpRtcpMutex);
    if (m_rtpRtcp) {
...
void VideoSendAdaptorImpl::OnReceivedIntraFrameRequest(uint32_t ssrc) {
